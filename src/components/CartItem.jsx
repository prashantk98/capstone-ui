...
export default function CartItem(prop) {
  ...
  const [value, setValue] = useState(prop.item.quantity);
...
<div className="cart-item" key={prop.index} >
        <img src={'data:image/jpeg;base64,'+prop.item.imgSrc} alt={prop.item.imgSrc} />
        <div className="cart-item__details">
          <h3 className="cart-item__title">{prop.item.productName}</h3>
          <p className="cart-item__description">Description Of {prop.item.productName}</p>
        </div>
        <p className="cart-item__detection-probability" style={{ color: prop.item.probability*100 <= 50 ? 'red' : prop.item.probability*100 <=70 ? 'orange' : 'green' }}>{(prop.item.probability*100).toFixed(2)}%</p>
        <div className="cart-item__quantity">
          <IconButton size="large" onClick={()=>{
            handleDecrement();
            prop.deleteQuantity(prop.index);
